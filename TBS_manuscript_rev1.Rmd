...
This study aims at replicating and strengthening the evidence --- that no EC is found when participants lack memory --- with higher statistical power, and in a simplified design (i.e., without nonpaired and valent stimuli) that is more comparable to previous studies.

In addition, valence-memory task type was manipulated:
In a first step towards reconciling the present findings with those by Waroquier et al. (), this study also realizes a condition that replicates the valence-assessment-with-attribution (VMA) task used by Waroquier et al, who found EC effects when participants reported having no memory (i.e., random guessing).
In addition, it explores an extension of the TBS towards (a) more than two subjective states and (b) continuous evaluative rating scales (that can be more informative than binary pleasant-unpleasant responses).
In sum, we implemented a between-subjects manipulation of task type with three levels: Participants completed either the TBS, VMA, or 3BS task.
...
We recruited participants until there were at least 180 participants who had passed the instruction-understanding checks. Of these 245 participants, 21 were excluded because they failed the seriousness or attention checks (i.e., they either indicated that their data should not be used for scientific purposes, or that they were not able to pay sustained attention during the learning phase). We additionnally excluded one participant because of a problem in the learning phase's stimulus presentation. This resulted in a final sample size of 223 participants (76 participants in the TBS condition; 78 in the VMA condition; 69 in the 3BS condition). 
